...
class AccentDictionaryParser:
    def __init__(self, exporter, UEManager, adjustVerbs, separateWord, separateVerbPhrase, ignoreVerbs, dontCombineDict,
                 skipList, parseWithMecab, verbToNoun):
    ...
    def checkSeparation(self):
        ...
        if len(self.results) > self.idx + 1 and self.results[self.idx][0] in self.verbToNoun and \
                self.results[self.idx + 1][0] == self.verbToNoun[self.results[self.idx][0]]:
    ...
    def dictBasedParsing(self, results, text, individual=False, configList=False, specifiedReading=False):
        ...
        if configList:
            ...
            self.kanaMode, self.dictMode, self.pitchMode, self.audioMode, self.graphMode = self.getKanaDictPitch(
                individual)
        if (
    ...
    def checkIgnoreValue(self):
        ...
        if val[0] in 'EOS' or (val[7] == '*' and (val[2] != '数' and val[0] not in [u'此', u'其'])) or val[1] == u'記号' or \
                val[0] in [u'の', u'で', u'た', u'に']:
            if val[0] != 'EOS':
                self.fStr += val[0]
            self.idx += 1
            return self.checkProceed()
    ...
    def processVerbAdjectives(self):
        ...
        if val[0] == u'した' or (((val[1] == u"動詞" or val[1] == u'形容詞') and val[6] != u"基本形"
            ...
            if not vAdjusted and word[-1:] != u'さ':
                yomi, pitchAr, pitchNumAr, self.audioGraph = self.dictionary.initSearch(word, self.prevW, self.nextW,
                                                                                        self.audioMode, self.graphMode,
                                                                                        val[2], self.specifiedReading)
            if not yomi:
                yomi, pitchAr, pitchNumAr, self.audioGraph = self.dictionary.initSearch(baseWord, self.prevW,
                                                                                        self.nextW, self.audioMode,
                                                                                        self.graphMode, val[2],
                                                                                        self.specifiedReading)
            if yomi:
    ...
    def processAlteredWord(self, altered):
        if self.kanaMode:
            if self.results[self.idx - 1][0][-altered:] in self.dictionary.suffixDict:
                word = self.okuriganer(self.results[self.idx - 1][0][-altered:] + '[' + self.dictionary.suffixDict[
                    self.results[self.idx - 1][0][-altered:]] + ']')
            else:
                word = self.results[self.idx - 1][0][-altered:]
                yomi, pitchAr, pitchNumAr, self.audioGraph = self.dictionary.initSearch(word, False, False, False,
                                                                                        False, False,
                                                                                        self.specifiedReading)
                if yomi:
    ...
    def processCompoundWord(self, compound, compAr):
        ...
        if re.match(u'^[\u3040-\u309f]+$', compound) or re.match(u'^[\u30a0-\u30ff]+$', compound):
            self.fStr += ' ' + self.okuriganer(
                compound + '[' + self.checkMode(self.dictMode, kihonkei) + self.checkMode(self.pitchMode,
                                                                                          ';' + pitches) + ']') + ' '
        else:
            if self.kanaMode:
                word = self.okuriganer(
                    compound + '[' + compAr[2] + self.checkMode(self.dictMode, kihonkei) + self.checkMode(
                        self.pitchMode, ';' + pitches) + ']')
            else:
                word = self.okuriganer(
                    compound + '[' + self.checkMode(self.dictMode, kihonkei) + self.checkMode(self.pitchMode,
                                                                                              ';' + pitches) + ']')
    ...
    def processWord(self):
        val = self.val
        if val[1] == u'記号' or val[1] == u'助詞' or re.match(u'^[\u3040-\u309f]+$', val[0]) or re.match(
                u'^[\u30a0-\u30ff]+$', val[0]):
            self.fStr += ' ' + val[0] + ' '
        else:
    ...
    def attemptProcessCompoundWord(self):
        compound, compAr = self.checkCompound(self.idx, self.val, self.results, self.audioMode, self.graphMode,
                                              self.val[2], self.specifiedReading)
        if compound:
...
class AccentExporter:
    def __init__(self, mw, aqt, UEManager, dictionary, addon_path, adjustVerbs, separateWord, separateVerbPhrase,
                 ignoreVerbs, dontCombineDict, skipList, parseWithMecab, verbToNoun):
        self.mw = mw
        self.aqt = aqt
        self.dictionary = dictionary
        self.addon_path = addon_path
        self.dictParser = AccentDictionaryParser(self, UEManager, adjustVerbs, separateWord, separateVerbPhrase,
                                                 ignoreVerbs, dontCombineDict, skipList, parseWithMecab, verbToNoun)
            ...
            if len(parsed) > 1 and "[" in parsed[1]:
                ...
                if parsed != ogString:
                    if yomi:
                        ...
                        newHTML = self.returnEntities(text.replace(rawString, rawString.replace(ogString,
                                                                                                re.sub(r';[^\[\]]*]',
                                                                                                       parsed,
                                                                                                       ogString.replace(
                                                                                                           '[a',
                        ...
                        newHTML = self.returnEntities(
                            text.replace(rawString, rawString.replace(parsed, ' ' + parsed + ' '))).replace('--IND--',
                                                                                                            '')
                    else:
                        newHTML = self.returnEntities(text.replace(rawString,
                                                                   rawString.replace(re.sub(r'\[.*]', '', parsed),
                                                                                     ' ' + parsed + ' '))).replace(
                            '--IND--', '')
                    editor.web.eval(
                        self.commonJS + self.insertHTMLJS % self.convertMalformedSpaces(newHTML).replace('"', '\\"'))
        if audioGraphList:
            self.addVariants(audioGraphList, note, editor)
            # self.reloadEditor()
    ...
    def addVariants(self, audioGraphs, note, editor=False, additionType=False):
        ...
        for aField in aFields:
            if self.dictParser.graphMode and aField in gFields and (aField in fields or aField.lower() == 'clipboard'):
                ...
                self.writeAudioGraphsText(audioGraphs, note, aField, additionType, gSep, 2, editor)
            else:
                if not additionType:
                    additionType = audioCon[1]
                if aField in fields or aField.lower() == 'clipboard':
                    self.writeAudioGraphsText(audioGraphs, note, aField, additionType, aSep, 0, editor)
        for gField in gFields:
            if (gField in fields or gField.lower() == 'clipboard') and gField not in aFields:
                if not additionType:
                    additionType = graphCon[1]
                self.writeAudioGraphsText(audioGraphs, note, gField, additionType, gSep, 1, editor)
