...
class AutoCSSJSHandler:
    def __init__(self, mw, addon_path):
        ...
        self.jCssHeader = '/*###TOGU JAPANESE SUPPORT CSS STARTS###\nDo Not Edit If Using Automatic CSS and JS Management*/'
        self.jCssFooter = '/*###TOGU JAPANESE SUPPORT CSS ENDS###*/'
        self.jCssHeaderP = '\/\*###TOGU JAPANESE SUPPORT CSS STARTS###\nDo Not Edit If Using Automatic CSS and JS Management\*\/'
        self.jCssFooterP = '\/\*###TOGU JAPANESE SUPPORT CSS ENDS###\*\/'
        self.jHistHeader = '<!--###TOGU JAPANESE SUPPORT CONVERTER JS START###\nDo Not Edit If Using Automatic CSS and JS Management-->'
        self.jHistFooter = '<!--###TOGU JAPANESE SUPPORT CONVERTER JS ENDS###-->'
        self.jJSHeader = '<!--###TOGU JAPANESE SUPPORT JS START###\nDo Not Edit If Using Automatic CSS and JS Management-->'
        self.jJSFooter = '<!--###TOGU JAPANESE SUPPORT JS ENDS###-->'
        self.jKHeader = '<!--###TOGU JAPANESE SUPPORT KATAKANA CONVERTER JS START###\nDo Not Edit If Using Automatic CSS and JS Management-->'
        self.jKFooter = '<!--###TOGU JAPANESE SUPPORT KATAKANA CONVERTER JS ENDS###-->'
    ...
    def checkProfile(self):
        ...
        if self.mw.pm.name in config['Profiles'] or (
                'all' in config['Profiles'] or 'All' in config['Profiles'] or 'ALL' in config['Profiles']):
            return True
        return False
            ...
            if model['name'] in self.wrapperDict:
                ...
                for idx, t in enumerate(model['tmpls']):
                    ...
                    if self.templateInModelDict(t['name'], modelDict):
                        ...
                        t['qfmt'], t['afmt'] = self.cleanFieldWrappers(t['qfmt'], t['afmt'], model['flds'],
                                                                       templateDict)
    ...
    def injectJapaneseConverterToTemplate(self, t):
        ...
        if converter not in ['both', 'kanji', 'kana', 'off']:
            miInfo(
                '"HistoricalConversion" has an invalid value in the config. Ensure that its value is one of the following: "both/kanji/kana/off".',
                level="err")
            return t
        elif converter in ['both', 'kanji', 'kana']:
            converterJS = self.getHistoricalConverterJs(converter, katakana)
            t['qfmt'] = self.newLineReduce(
                self.removeKataverterJs(self.removeConverterJs(t['qfmt'])) + '\n\n' + converterJS)
            t['afmt'] = self.newLineReduce(
                self.removeKataverterJs(self.removeConverterJs(t['afmt'])) + '\n\n' + converterJS)
            return t
        if katakana == 'true':
    ...
    def checkVariantSyntax(self):
        ...
        for variant in ['PitchGraphFields', 'AudioFields']:
            ...
            if len(varAr) not in [2, 3]:
                syntaxErrors += '\nThe "' + variant + '" configuration "' + config[
                    variant] + '" is incorrect. The syntax is invalid.'
            else:
                fields = varAr[0].split(',')
                for field in fields:
                    if field.lower() not in ['clipboard', 'none', ''] and not self.fieldExists(field):
                        syntaxErrors += '\nThe "' + variant + '" configuration "' + config[
                            variant] + '" is incorrect. The "' + field + '" field does not exist in your collection.'
                        fieldErrors.append(field)
                if varAr[1].lower() not in ['overwrite', 'add', 'no']:
                    syntaxErrors += '\nThe "' + variant + '" configuration "' + config[
                        variant] + '" is incorrect. Please ensure that second value is either "overwrite", "add", or "no".'
        if syntaxErrors != '':
            if len(fieldErrors) > 0:
                if miAsk(
                        'Please make sure the syntax is as follows "field,field;type(;separator)". The syntax is incorrect for the following entries:' + syntaxErrors + '\n\n Would you like to delete these fields from you configuration?'):
    ...
    def getWrapperDict(self):
        wrapperDict = {}
        displayOptions = ['hover', 'coloredhover', 'kanji', 'coloredkanji', 'reading', 'coloredreading', 'kanjireading',
                          'coloredkanjireading']
        models = self.mw.col.models.all()
        config = self.getConfig()
        syntaxErrors = ''
                    ...
                    if success:
                        ...
                        if conflictFree:
                            if dataArray[2] not in wrapperDict:
                                ...
                                if [dataArray[3], dataArray[4], dataArray[5], displayOption] not in wrapperDict[
                                    dataArray[2]]:
                                    alreadyIncluded.append([dataArray, displayOption])
                                    wrapperDict[dataArray[2]].append(
                                        [dataArray[3], dataArray[4], dataArray[5], displayOption])
                        else:
