...
*AutoCssJsGeneration*: "on" or "off", tells the add-on whether to keep config options in sync with card template's CSS and Javascript. Leave this on unless you understand how to edit these areas yourself.
...
*FuriganaFontSize*: A number from 1 to 10, without parentheses, this is the font-size of the furigana relative to the size of the regular font. For example 4 is equal to 40% the size of the regular font, 10 is 100%.
...
The syntax is on/off;on/off;on/off;on/off;on/off, each option can be toggled by setting it to "on" or "off"
Kana is whether to export the regular kana readings for a word.
DictForm is whether to export an adjective or verb's dictionary form, note that a dictionary form is necessary in order to display a pitch graph for verbs as pitch graphs for conjugations are not supported at the current time.
...
*KatakanaConversion*: "on" or "off", whether to convert your card's text to katakana.
