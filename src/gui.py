...
class JSGui(QScrollArea):
    def __init__(self, mw, colArray, languageModeler, reboot, CSSJSHandler, UEManager):
        ...
        self.sides = {'Front': 'Front: Applies the display type to the front of the card.',
        ...
        self.displayTypes = {'Kanji': ['kanji', 'Kanji: Displays text without accent coloring or furigana.'],
                             'Colored Kanji': ['coloredkanji',
                                               'Colored Kanji: Displays text with accent coloring but no furigana.'],
                             'Hover': ['hover',
                                       'Hover: Displays text without accent coloring or furigana,\nbut displays an individual word\'s furigana when it is hovered.'],
                             'Colored Hover': ['coloredhover',
                                               'Colored Hover: Displays text without accent coloring or furigana,\nbut displays an individual word\'s accent coloring and furigana when it is hovered.'],
                             'Kanji Reading': ['kanjireading',
                                               'Kanji Reading: Displays text without accent coloring but with furigana.'],
                             'Colored Kanji Reading': ['coloredkanjireading',
                                                       'Colored Kanji Reading: Displays text with accent coloring and furigana.'],
                             'Reading': ['reading',
                                         'Reading: Displays text in kana without accent coloring.\nNote that if a word\'s kana reading is not available it will be displayed in kanji.'],
                             'Colored Reading': ['coloredreading',
                                                 'Colored Reading: Displays text in kana with accent coloring.\nNote that if a word\'s kana reading is not available it will be displayed in kanji.']}
        ...
        self.addToguNoteTypeOnApply = False
    ...
    def setInitialValues(self):
        # self.setWindowIcon(QIcon(join(addon_path, 'icons', '.png')))
        self.setWindowTitle("Togu Japanese Settings (Ver. " + verNumber + ")")
    ...
    def removeMultipleRules(self, rows, length):
        if miAsk(
                'Are you sure you would like to remove the ' + length + ' rules that are selected from the overwrite rule list?'):
    ...
    def addRule(self):
        ...
        added, addId = self.ueMng.addRule(original, overwrite, self.ui.ncAddCB.isChecked(), self.ui.lcAddCB.isChecked(),
                                          self)
        if added:
    ...
    def loadCSSJSAddTogu(self):
        if self.config['AutoCssJsGeneration'].lower() == 'on':
            self.ui.autoCSSJS.setChecked(True)
        if self.config['AddToguJapaneseTemplate'].lower() == 'on':
            self.ui.addToguNoteType.setChecked(True)

    def initHandlers(self):
        self.ui.autoCSSJS.toggled.connect(self.handleAutoCSSJS)
        self.ui.addToguNoteType.toggled.connect(self.handleAddTogu)
        ...
        self.ui.profilesPB.clicked.connect(
            lambda: self.addRemoveFromList(self.ui.profilesCB.currentText(), self.ui.profilesPB, self.ui.profilesList,
                                           self.selectedProfiles, True))
        self.ui.profilesCB.currentIndexChanged.connect(
            lambda: self.profAudioGraphChange(self.ui.profilesCB.currentText(), self.ui.profilesPB,
                                              self.selectedProfiles))
        self.ui.audioFieldsPB.clicked.connect(
            lambda: self.addRemoveFromList(self.ui.audioFieldsCB.currentText(), self.ui.audioFieldsPB,
                                           self.ui.audioFieldsList, self.selectedAudioFields, True))
        self.ui.audioFieldsCB.currentIndexChanged.connect(
            lambda: self.profAudioGraphChange(self.ui.audioFieldsCB.currentText(), self.ui.audioFieldsPB,
                                              self.selectedAudioFields))
        self.ui.pitchGraphsPB.clicked.connect(
            lambda: self.addRemoveFromList(self.ui.pitchGraphsCB.currentText(), self.ui.pitchGraphsPB,
                                           self.ui.pitchGraphsList, self.selectedGraphFields, True))
        self.ui.pitchGraphsCB.currentIndexChanged.connect(
            lambda: self.profAudioGraphChange(self.ui.pitchGraphsCB.currentText(), self.ui.pitchGraphsPB,
                                              self.selectedGraphFields))
    ...
    def exportRules(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self, "Save Overwrite Rules List", 'overwriterules.json',
    ...
    def importRules(self):
        ...
        if fileName:
            ...
            laiCollideRB.setToolTip(
                'Any rules that are found to have the same original value as an existing rule in your \ncurrent list are ignored and not added to the combined list.')
            ovCollideRB.setToolTip(
                'Overwrites any rules that have the same original value with the values in the imported list.')
            ...
            importButton.clicked.connect(lambda: self.ruleListImport(self.importW,
                                                                     fileName, combineRB.isChecked(),
                                                                     ovCollideRB.isChecked()))
            self.importW.show()
        ...
        if imported is not False:
            if not combine:
                ...
                if overwriteCollides:
                    miInfo(str(imported[0]) + ' rules have been imported.<br>' + str(
                        imported[1]) + ' rules have been overwritten.', level='not')
                else:
                    miInfo(str(imported[0]) + ' rules have been imported.<br>' + str(
                        imported[1]) + ' rules have been ignored because they conflicted with existing rules.',
                           level='not')
            self.ueMng.setupModel(self)
    ...
    def handleAddTogu(self):
        if self.ui.addToguNoteType.isChecked():
            if not self.checkToguNoteExistence():
                self.addToguNoteTypeOnApply = True
                self.resetToguActiveFields()
    ...
    def loadBehaviorOptions(self):
        ...
        if self.config['HistoricalConversion'].lower() == 'kanji' or self.config[
            'HistoricalConversion'].lower() == 'both':
            self.ui.historicalKanji.setChecked(True)
        if self.config['HistoricalConversion'].lower() == 'kana' or self.config[
            'HistoricalConversion'].lower() == 'both':
    ...
    def resetToguActiveFields(self):
        self.removeToguFields()
        self.addToguFields()

    def checkToguNoteExistence(self):
        models = self.mw.col.models.all()
        for model in models:
            if model['name'] == 'Togu Japanese':
                return True
        return False

    def removeToguFields(self):
        afList = self.ui.listWidget
        for i in reversed(range(afList.rowCount())):
            if afList.item(i, 1).text() == 'Togu Japanese':
                self.ui.listWidget.removeRow(i)

    def addToguFields(self):
        self.addToList('All', 'Togu Japanese', 'Sentence', 'Expression', 'Front', 'Colored Hover')
        self.addToList('All', 'Togu Japanese', 'Sentence', 'Expression', 'Back', 'Colored Kanji Reading')
        self.addToList('All', 'Togu Japanese', 'Sentence', 'Meaning', 'Back', 'Colored Hover')
    ...
    def handleAutoCSSJS(self):
        if self.ui.autoCSSJS.isChecked():
            ...
            self.ui.addToguNoteType.setEnabled(True)
            ...
            self.ui.addToguNoteType.setEnabled(False)
    ...
    def loadCurrentAFs(self):
        ...
        for af in afs:
            ...
            if dt in self.displayTranslation:
                ...
                if prof == 'all':
                    prof = 'All'
                self.addToList(prof, afl[2], afl[3], afl[4], afl[5][0].upper() + afl[5][1:].lower(),
                               self.displayTranslation[dt])

    def saveCSSJSAddTogu(self):
        togu = 'off'
        css = 'off'
        if self.ui.autoCSSJS.isChecked():
            css = 'on'
    ...
    def saveConfiguration(self):
        ...
        addtogu, bo, autocss, ds, goh, gohb, kc, poc = self.saveBinaryOptions()
        newConf = {"ActiveFields": self.saveActiveFields(), "Individual:Kana;DictForm;Pitch;Audio;Graphs": wc,
                   "Group:Kana;DictForm;Pitch;Audio;Graphs": sc,
                   "FuriganaFontSize": ffs, "LookAhead": la, "Profiles": self.saveProfilesConfig(),
                   "AudioFields": ac, "PitchGraphFields": gc, "ColorsHANOK": colors,
                   "AddToguJapaneseTemplate": addtogu, "BufferedOutput": bo,
                   "AutoCssJsGeneration": autocss, "DisplayShapes": ds, "GraphOnHover": goh, "GraphOnHoverBack": gohb,
                   "KatakanaConversion": kc, "PlayAudioOnClick": poc,
                   "HistoricalConversion": self.saveHistoricalConversion()

                   }
        if self.addToguNoteTypeOnApply:
    ...
    def saveHANOK(self):
        return [self.ui.heibanColor.text(), self.ui.atamadakaColor.text(), self.ui.nakadakaColor.text(),
                self.ui.odakaColor.text(), self.ui.kifukuColor.text()]
    ...
    def loadSelectedRow(self, row, col):
        ...
        if loaded:
            self.initEditMode()
            self.selectedRow = [afList.item(row, 0), afList.item(row, 1), afList.item(row, 2), afList.item(row, 3),
                                afList.item(row, 4), afList.item(row, 5)]
    ...
    def dupeRow(self, afList, profile, nt, ct, field, side, dt, selRow=False):
        for i in range(afList.rowCount()):
            if selRow is not False:
                if i == selRow[0].row():
                    continue
            if (afList.item(i, 0).text() == profile or afList.item(i,
    ...
    def profileChange(self):
        ...
        if self.ui.activeProfileCB.currentIndex() == 0:
            ...
            for noteType in self.ciSort(self.cA[prof]):
                ...
                self.ui.activeNoteTypeCB.setItemData(self.ui.activeNoteTypeCB.count() - 1,
                                                     noteType + ' (Prof:' + prof + ')', Qt.ToolTipRole)
    ...
    def setToolTips(self):
        ...
        self.ui.autoCSSJS.setToolTip(
            'If checked, the addon will manage the CSS and JS of all note and card types designated in the active fields list below.\nIf this is disabled the user is responsible for ensuring that their CSS and JS functions as they wish.')
        self.ui.addToguNoteType.setToolTip(
            'If checked, the addon will attempt to add the Togu Japanese Note Type if it does not already exist.\nIt will also regenerate the active fields for the Togu Japanese Note Type.')
        self.ui.profilesCB.setToolTip(
            'These are the profiles that the add-on will be active on.\nWhen set to "All", the add-on will be active on all profiles.')
        self.ui.sentenceKana.setToolTip(
            'When checked, the addon will generate the kana reading of all\nrecognized words within the target field.')
        self.ui.sentenceDictForm.setToolTip(
            'When checked, the addon will generate the dictionary form of all\nrecognized verbs and adjectives within the target field.')
        self.ui.sentenceAudio.setToolTip(
            'When checked, the addon will export the audio when available, for all\nrecognized words within the target field, into the selected "Audio Field(s)".')
        self.ui.sentenceGraphs.setToolTip(
            'When checked, the addon will export the pitch graph(s) when available, for all\nrecognized words within the target field, into the selected "Pitch Graph Field(s)".')
        self.ui.sentenceAccents.setToolTip(
            'When checked, the addon will export the pitch accent(s) when available, for all\nrecognized words within the target field.')
        self.ui.wordKana.setToolTip(
            'When checked, the addon will generate the kana reading for the next\nrecognized word within the highlighted text, or after the cursor\'s current position\nwithin a field if no text is currently highlighted.')
        self.ui.wordDictForm.setToolTip(
            'When checked, the addon will generate the dictionary form for the next\nrecognized adjective or verb within the highlighted text, or after the cursor\'scurrent position\nwithin a field if no text is currently highlighted.')
        self.ui.wordAudio.setToolTip(
            'When checked, the addon will generate the audio for the next\nrecognized word within the highlighted text, or after the cursor\'s current position\nwithin a field if no text is currently highlighted. The audio will be exported into the selected "Audio Field(s)".')
        self.ui.wordGraphs.setToolTip(
            'When checked, the addon will generate the pitch graph(s) for the next\nrecognized word within the highlighted text, or after the cursor\'s current position\nwithin a field if no text is currently highlighted. The audio will be exported into the selected "Pitch Graph Field(s)".')
        self.ui.wordAccents.setToolTip(
            'When checked, the addon will generate the pitch accent(s)for the next\nrecognized adjective or verb within the highlighted text, or after the cursor\'s current position\nwithin a field if no text is currently highlighted.')
        self.ui.audioFieldsCB.setToolTip(
            'When audio generation is enabled the audio will be pasted within the selected audio field(s).\nNote that if a selected audio field has also been designated as a pitch graph field\nthat the pitch graph(s) will preceed the audio, and the pitch graph\'s separator will be used.')
        self.ui.audioAdd.setToolTip(
            'Audio will be added on to the selected audio field(s) following the separator.\nThe default separator is an html line break "<br>".')
        self.ui.audioOverwrite.setToolTip(
            'Audio will be generated into the selected audio field(s),\noverwriting their current contents.')
        self.ui.audioIfEmpty.setToolTip('Audio will be only be added to the selected audio field(s) if they are empty.')
        self.ui.audioSep.setToolTip(
            'This is the separator to be used when adding audio to the selected audio field(s)\' current contents.\nThe default separator is an html line break "<br>".')
        self.ui.pitchGraphsCB.setToolTip(
            'When pitch graph generation is enabled the pitch graph(s) will be pasted within the selected pitch graph field(s).\nNote that if a selected audio field has also been designated as a pitch graph field\nthat the pitch graph(s) will preceed the audio, and the pitch graph\'s separator will be used.')
        self.ui.graphAdd.setToolTip(
            'Pitch graoph(s) will be added on to the selected pitch graph field(s) following the separator.\nThe default separator is an html line break "<br>".')
        self.ui.graphOverwrite.setToolTip(
            'Pitch graph(s) will be generated into the selected pitch graph field(s),\noverwriting their current contents.')
        self.ui.graphIfEmpty.setToolTip(
            'Pitch graph(s) will be only be added to the selected pitch graph field(s) if they are empty.')
        self.ui.graphSep.setToolTip(
            'This is the separator to be used when adding pitch graph(s) to the selected pitch graph field(s)\' current contents.\nThe default separator is an html line break "<br>".')
        self.ui.lookAhead.setToolTip(
            'This is the number of characters within which the addon will attempt to search for a recognizable word when using the "語" button, a larger number will be slightly slower.')
        ...
        self.ui.furiganaFontSize.setToolTip(
            'This is the size setting of all displayed furigana.\nThe size is in relation to the percentage of the base word\'s, for\nexample 1 is 10\% of the baseword\' size, 2 is 20\%, and so on.')
        self.ui.bufferedOutput.setToolTip(
            'The addon will load a card\'s text in chunks. This option is for very slow machines or \nuseful if a user has lots of cards with a huge amount of text. This option is\nexperimental and it is recommended that the user keeps text on their cards to a moderate length.')
        self.ui.displayShapes.setToolTip(
            'If checked, pitch shapes signifying a word\'s alternate pitch accents will be displayed after\na word on the "coloredkanji", "coloredreading", and "coloredkanjireading" display type options.')
        self.ui.graphOnHover.setToolTip(
            'If checked, the addon will display pitch accent graphs when hovering over a word on the front of the card.')
        self.ui.graphOnHoverBack.setToolTip(
            'If checked, the addon will display pitch accent graphs when hovering over a word on the back of the card.\n##YOUR NOTE TYPE MUST NOT INCLUDE {{FrontSide}} ON ITS BACK TEMPLATE FOR THIS TO FUNCTION CORRECTLY.##')
        self.ui.audioOnClick.setToolTip(
            'If checked, audio will be played when clicking a word if it is available.\nNote that the audio will only be played for the first occurrence of the word within the pitch accent dictionary.')
        ...
        self.ui.ncAddCB.setToolTip(
            'If selected, the rule to be added will be applied to the active fields \nin all new cards in your collection.')
        self.ui.lcAddCB.setToolTip(
            'If selected, the rule to be added will be applied to the active fields \nin all cards you are currently studying in your collection.')
        self.ui.ncAllCB.setToolTip(
            'If selected, all current overwrite rules will be applied to the active fields \nin all new cards in your collection.')
        self.ui.lcAllCB.setToolTip(
            'If selected, all current overwrite rules will be applied to the active fields \nin all cards you are currently studying in your collection.')
        self.ui.addRuleButton.setToolTip(
            'Adds a new overwrite rule and applies it to the collection if a checkbox is checked. \nRules are only applied to fields that have been designated an "active field" in the active fields tab.')
        self.ui.runRulesButton.setToolTip(
            'Applies all overwrite rules to your collection. Rules are only applied to fields \nthat have been designated an "active field" in the active fields tab.')
        self.ui.importRules.setToolTip(
            'Import an overwrite rules list. Options to overwrite or ignore duplicates, and to \noverwrite the current rules list with the newly imported list.')
        self.ui.exportRules.setToolTip('Export your current overwrite rules list.')
        self.ui.removeLegacy.setToolTip(
            'When this box is checked-off and the "Apply" button is clicked,\nthen all legacy MIA JS & CSS will be removed from all your card types.')
