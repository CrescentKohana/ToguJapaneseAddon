...
class OgOvFilter(QSortFilterProxyModel):
    ...
    def testData(self, text):
        ...
        self.sourceModel().ueList = sorted(foundOriginal, key=lambda x: len(x[0])) + sorted(foundOverwrite,
                                                                                            key=lambda x: len(
                                                                                                x[1])) + notFound
...
class UserExceptionManager:
    ...
    def openAddMenu(self, editor=False, text=False):
        ...
        # self.addMenu.setWindowIcon(QIcon(join(self.addon_path, 'icons', '')))
    ...
    def addRuleAndClearText(self):
        if self.addRule(self.addMenu.ui.originalLE.text(), self.addMenu.ui.overwriteLE.text(),
                        self.addMenu.ui.ncAddCB.isChecked(), self.addMenu.ui.lcAddCB.isChecked(), self.addMenu, True):
    ...
    def addRule(self, original, overwrite, newCards, learnedCards, parentWidget, addMenu=False):
        ...
        if foundId is not False:
            if not miAsk('The rule "' + original + '" => "' + self.ueList[foundId][1] +
                         '" already overwrites the given text. Would you like to overwrite it with your new rule?'):
                return False, False
            else:
                edit = True
        if edit:
    ...
    def ruleExists(self, original, message=False):
        ...
        for idx, ogOv in enumerate(self.ueList):
            if original == ogOv[0]:
                if message:
                    miInfo('A rule with this original value already exists ("' + original + '" => "' + self.ueList[idx][
                        1] + '""), please ensure that you are defining a unique value.', level='err')
                return idx
        return False
    ...
    def applyRules(self, ruleList, newCards, learnedCards, parentWidget, notes=False, message=False):
        ...
        self.mw.reset()
        progWid.hide()
        miInfo('Rule(s) have been applied ' + str(appliedRules) + ' times.<br>' + str(
            altered) + ' notes have been altered.', parent=parentWidget, level='not')
        ...
        try:
            ...
            if combine:
                ...
                for ogOv in newList:
                    if len(ogOv) != 2:
                        miInfo(
                            'The overwrite rules list could not be imported. Please make sure the target file is a valid overwrite rules list and try again.',
                            level='err')
                        return False
                    original = ogOv[0]
                    overwrite = ogOv[1]
